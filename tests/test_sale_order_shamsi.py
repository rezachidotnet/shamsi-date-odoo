...
class TestSaleOrderShamsi(TransactionCase):
    ...
    def test_known_bug_expected_date_is_absent_from_dependency_graph(self):
        """KNOWN_BUG_EXPECTED_DATE_DEPENDS: capture the current incomplete graph."""
        ...
        self.assertNotIn("expected_date", dependencies)

    @skip("KNOWN_BUG_EXPECTED_DATE_DEPENDS: future behavior specification")
    def test_known_bug_expected_date_should_be_a_dependency(self):
        """KNOWN_BUG_EXPECTED_DATE_DEPENDS: desired future regression specification."""
        model = self.env["sale.order"]
        field = model._fields["expected_date_shamsi"]
        dependencies, _context_dependencies = field.get_depends(model)
        self.assertIn("expected_date", dependencies)

    def test_current_datetime_conversion_ignores_user_timezone(self):
        """KNOWN_BUG_USER_TIMEZONE: current output uses the stored UTC calendar day."""
    ...
    @skip("KNOWN_BUG_USER_TIMEZONE: future behavior specification")
    def test_known_bug_datetime_should_use_new_york_calendar_day(self):
        """KNOWN_BUG_USER_TIMEZONE: desired local date is still 1402/12/29 in New York."""
        order = self.env["sale.order"].with_context(tz="America/New_York").new({
            "partner_id": self.partner.id,
            "date_order": datetime(2024, 3, 20, 0, 30, 0),
        })
        order._compute_shamsi_dates()
        self.assertEqual(order.date_order_shamsi, "1402/12/29")
